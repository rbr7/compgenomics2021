...
class full_pipeline_form(forms.Form):
    name = forms.CharField(label='Enter your name', max_length=40)
    email = forms.EmailField(label='Enter your email')
    file = forms.FileField()

class view_results_form(forms.Form):
    pipeline_name = forms.CharField(label='Enter the name of the pipeline run [GA GP FA CG FULL]', max_length=10)
    jobId = forms.CharField(label='Enter the Job ID', max_length=15)
